...
void InitPalette() {
  ...
  palette = vec3[](RGB(0, 0, 0), RGB(255, 255, 255), RGB(152, 75, 67),
                   RGB(121, 193, 200), RGB(155, 81, 165), RGB(104, 174, 92),
                   RGB(62, 49, 162), RGB(201, 214, 132), RGB(155, 103, 57),
                   RGB(106, 84, 0), RGB(195, 123, 117), RGB(85, 85, 85),
                   RGB(138, 138, 138), RGB(163, 229, 153), RGB(138, 123, 206),
                   RGB(173, 173, 173));

  // 8-Color metalic-like sub palette.
  subPalette = vec3[](palette[6], palette[11], palette[4], palette[14],
                      palette[5], palette[3], palette[13], palette[1]);
}
...
// Distance field functions & operations by iq.
...
float Scene(vec3 pos) {
  ...
  float grid = opU(opU(sdCylinder(rep.xyz, gSize), sdCylinder(rep.xzy, gSize)),
                   sdCylinder(rep.zxy, gSize));
...
vec3 Normal(vec3 pos) {
  ...
  vec3 normal = vec3(Scene(pos - offset.xyz) - Scene(pos + offset.xyz),
...
// Marches a ray defined by the origin and direction and returns the hit
// position.
...
vec3 Shade(vec3 position, vec3 normal, vec3 rayOrigin, vec3 rayDirection,
  ...
  // Specular highlight
  shade += 0.6 * max(0.0, dot(-reflect(normalize(position - lightPos), normal),
                              rayDirection));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 outColor = Shade(scenePosition, Normal(scenePosition), rayOrigin,
                        rayDirection, fragCoord / DOWN_SCALE);
